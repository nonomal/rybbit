...
export async function generateMetadata(props: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  ...
  const url = `https://docs.rybbit.io/updates/${params.slug.join("/")}`;
  ...
  return {
    ...
    keywords: page.data.tags
      ? [...page.data.tags, "product update", "Rybbit"]
      : ["product update", "Rybbit"],
    authors: page.data.author ? [{ name: page.data.author }] : undefined,
  };
}

export default async function UpdatePostPage(props: { params: Promise<{ slug: string[] }> }) {
  const params = await props.params;
  const page = updatesSource.getPage(params.slug);
