...
export const metadata: Metadata = {
  title: "Updates",
  description: "Product updates and announcements from Rybbit",
};

export default function UpdatesPage() {
  const posts = [...updatesSource.getPages()];
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16">
      <div className="mb-12">
        ...
        <p className="text-xl text-neutral-400">Product updates, new features, and announcements from the Rybbit team</p>
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {sortedPosts.map(post => {
          ...
          return (
            <Link key={post.slugs.join("/")} href={`/updates/${post.slugs.join("/")}`} className="group">
      ...
      {sortedPosts.length === 0 && (
        <div className="text-center py-12">
          <p className="text-neutral-400">No updates yet. Check back soon!</p>
        </div>
      )}
    </div>
  );
